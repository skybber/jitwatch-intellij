...
class JitWatchModelService(private val project: Project) {
    ...
    fun loadBytecode(file: PsiFile) {
        ...
        if (module != null) {
            ...
            for (cls in allClasses) {
                ...
                val metaClass =
                    ApplicationManager.getApplication().runReadAction(Computable { getMetaClass(cls) }) ?: continue
    ...
    private fun buildAllBytecodeAnnotations(metaClass: MetaClass, target: MutableMap<IMetaMember, BytecodeAnnotations>) {
        for (metaMember in metaClass.metaMembers) {
            val annotations = try {
                IJBytecodeAnnotationBuilder().buildBytecodeAnnotations(metaMember, 0, model)
            } catch (e: Exception) {
    ...
    private class IJBytecodeAnnotationBuilder : BytecodeAnnotationBuilder(false) {
    ...
    fun processBytecodeAnnotations(psiFile: PsiFile, callback: (method: PsiElement,
        ...
        for (cls in languageSupport.getAllClasses(psiFile)) {
            ...
            for (method in languageSupport.getAllMethods(cls)) {
                ...
                val membersWithAnnotations = annotations.members
                for (memberWithAnnot in membersWithAnnotations) {
                    val annotationList = annotations.getAnnotationList(memberWithAnnot)
